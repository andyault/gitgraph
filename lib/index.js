...
const lib = {
  ...
  buildSvg: graph => {
    ...
    const mergeLine = d3
      ...
      .x(commit => scale.x(commit.branchIndex + 0.5))
      .y(commit => scale.y(commit.index))
      .curve(d3.curveStepBefore);

    branches.each(function(branch, i) {
      //iterate through commits
      //if commit is merge/branch
      //insert commit before current commit with x="from branch index"
      //else, add x="branch index"
      const mergeCommits = [...branch.values];

      for (let j = mergeCommits.length - 1; j >= 0; j--) {
        const mergeCommit = mergeCommits[j];
        ...
        if (
          ...
          mergeCommit.branchIndex = 0;

          mergeCommits.splice(j, 0, {
            type: commitTypes.MERGE,
            index: mergeCommit.index - 1,
            branchIndex: branchIndices[mergeCommit.from] - i,
          });
        }
      }

      if (branch.key === 'to-staging') console.log(branch.key, mergeCommits);

      d3.select(this)
        .append('path')
        .attr('d', mergeLine(mergeCommits))
        .attr('fill', 'none')
        .attr('stroke', 'blue');
    });
