...
const lib = {
  ...
  buildSvg: graph => {
    ...
    const branches = main
      .selectAll('g')
      .data(data) //.filter(d => d.key !== '_meta'))
      .enter()
      .append('g')
      .attr('id', branch => branch.key)
      .attr('transform', (_, i) => `translate(${i * draw.BRANCH_WIDTH}, 0)`);

    //for each branch, add commits
    const branchMiddle = draw.BRANCH_WIDTH / 2 - draw.COMMIT_RADIUS / 2;
    ...
    const line = d3
      ...
      .x(branchMiddle)
      .y(commit => commit.index * draw.COMMIT_HEIGHT);
    ...
    const commits = branches
      ...
      .enter()
      .append('circle')
      .attr('cx', branchMiddle)
      .attr('cy', commit => commit.index * draw.COMMIT_HEIGHT)
