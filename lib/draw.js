...
const draw = {
  ...
  redraw: (main, data) => {
    ...
    //merge lines
    const mergeLines = drawUtil.onMergeLinesEnter(main);

    // const mergeLineBranches = mergeLines
    //   .selectAll('g.branch')
    //   .data(data, branch => branch.key)
    //   .join(enter => drawUtil.onMergeLinesBranchEnter(enter, scale));

    const mergeCommits = util.getMergeCommits(data);
    const mergeLine = drawUtil.createMergeLine(scale);

    mergeLines
      .selectAll('path.mergeLine')
      .data(mergeCommits)
      .join(enter => drawUtil.onMergeLinesUpdate(enter, mergeLine));
...
const drawUtil = {
  ...
  onMergeLinesBranchEnter: (selection, scale) => {
    return (
      selection
        .append('g')
        // .attr('id', branch => branch.key)
  ...
  onMergeLinesUpdate: (selection, line) => {
    return selection
      .append('path')
      .attr('class', 'mergeLine')
      .attr('d', mergeCommit => line(mergeCommit.values))
      .attr('fill', 'none')
      .attr('stroke', mergeCommit => mergeCommit.color);
  },

  //lines
  createMergeLine: scale => {
    return d3
      .line()
      .curve(d3.curveStep)
      .x(value => scale.x(value.x + 0.5))
      .y(value => scale.y(value.y));
  },
  ...
  //curves
  // curveRoundedStep = class curveRoundedStep {
  ...
  //paths
  oldcreateMergeLine: (fromX, fromY, toX, color = '#000', dashed = false) => {
