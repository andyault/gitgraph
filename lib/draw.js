  ...
  getCanvasSize: (context, shadowSelection, axis = draw.AXIS_HORIZONTAL) => {
    ...
    if (axis === draw.AXIS_HORIZONTAL) {
      ...
      size.height =
        ...
        draw.BRANCH_HEIGHT * numBranches * 2 +
        draw.PADDING_VERTICAL;
    } else {
      size.width =
        draw.PADDING_HORIZONTAL +
        draw.BRANCH_WIDTH * numBranches * 2 +
        draw.PADDING_HORIZONTAL;

      size.height =
        draw.PADDING_VERTICAL +
        draw.COMMIT_HEIGHT * numCommits * 2 +
        draw.PADDING_HORIZONTAL;
    }
          ...
          if (active && branched) {
            ...
            if (commitType !== gitGraph.commitTypes.EMPTY) {
              ...
              if (commitMessage) {
                ...
                if (isHorizontal) {
                  messageY =
                    (branches.length + 0.5) * draw.BRANCH_HEIGHT +
                    draw.PADDING_VERTICAL;
                } else {
                  messageX =
                    (branches.length + 0.5) * draw.BRANCH_WIDTH +
                    draw.PADDING_HORIZONTAL;
                }
