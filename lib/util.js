...
util.initCanvas = function initCanvas(shadowSelection, axis) {
  ...
  const { width, height } = draw.getCanvasSize(context, shadowSelection, axis);
...
util.renderGraph = function renderGraph(graph, axis) {
  ...
  const canvasInfo2 = util.initCanvas(shadowInfo.selection, draw.AXIS_VERTICAL);
  draw.drawGraph(canvasInfo2.context, shadowInfo.selection, draw.AXIS_VERTICAL);

  //create final output
  // const ret = createOutput(shadowInfo, canvasInfo);
  const ret = createOutput2(shadowInfo, canvasInfo, canvasInfo2);
...
const createOutput2 = (shadowInfo, canvasInfo1, canvasInfo2) => {
  const dom = new JSDOM();
  const { document } = dom.window;

  const img = document.createElement('img');
  img.style.border = '1px solid red';
  img.src = canvasInfo1.canvas.toDataURL();
  document.body.appendChild(img);

  const img2 = document.createElement('img');
  img2.style.border = '1px solid red';
  img2.src = canvasInfo2.canvas.toDataURL();
